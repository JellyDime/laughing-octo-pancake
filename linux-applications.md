### ArmorPaint

- Overview: A GPU-accelerated tool for physically-based 3D texture painting and material creation.
- Website: https://armorpaint.org/
- Source Code: https://github.com/armory3d/armortools
- Download: https://armorpaint.org/download

### Gale

- Overview: A lightweight mod manager for Thunderstore that installs, updates, and organizes mods and modpacks.
- Website: https://kesomannen.com/gale
- Source Code: https://github.com/Kesomannen/gale
- Download: https://github.com/Kesomannen/gale/releases

### Limo

- Overview: A general-purpose mod manager with NexusMods API and LOOT integration, for managing, sorting, and deploying game mods.
- Source Code: https://github.com/limo-app/limo
- Download: https://flathub.org/en/apps/io.github.limo_app.limo

### Material Maker

- Overview: A procedural PBR material editor and texture-painting tool for creating and exporting game-engine materials.
- Website: https://www.materialmaker.org/
- Source Code: https://github.com/RodZill4/material-maker
- Download: https://rodzilla.itch.io/material-maker

### Scribus

- Overview: Provides professional page layout and press‑ready output for designing print‑ready documents.
- Website: https://www.scribus.net/
- Source Code: https://sourceforge.net/projects/scribus/
- Download: https://flathub.org/en/apps/net.scribus.Scribus

### Friction

- Overview: A motion graphics tool for creating vector and raster animations for web and video.
- Website: https://friction.graphics/
- Source Code: https://github.com/friction2d/friction
- Download: https://friction.graphics/download.html

### Blender

- Overview: Provides integrated 3D creation tools for modeling, animation, rendering, compositing, and video editing.
- Website: https://www.blender.org/
- Source Code: https://projects.blender.org/blender/blender
- Download: https://www.blender.org/download/

### Frappe Builder

- Overview: A low-code tool to visually design and publish fast, dynamic websites with drag-and-drop components and built-in CMS features.
- Website: https://frappe.io/builder
- Source Code: https://github.com/frappe/builder

### Publii

- Overview: A desktop-based CMS for building and managing static websites with a visual interface and one-click deployment.
- Website: https://getpublii.com/
- Source Code: https://github.com/GetPublii/Publii
- Download: https://getpublii.com/download/

### Sober

- Overview: Lets users run Roblox experiences on Linux via a specialized Roblox-focused runtime.
- Website: https://sober.vinegarhq.org/
- Download: https://flathub.org/en/apps/org.vinegarhq.Sober

### Vinegar

- Overview: A fast, configurable bootstrapper for running Roblox Studio on Linux via Wine.
- Website: https://vinegarhq.org/
- Source Code: https://github.com/vinegarhq/vinegar
- Download: https://flathub.org/en/apps/org.vinegarhq.Vinegar

### LibreOffice

- Overview: A comprehensive office suite providing word processing, spreadsheets, presentations, and more.
- Website: https://www.libreoffice.org/
- Source Code: https://github.com/LibreOffice/core
- Download: https://flathub.org/en/apps/org.libreoffice.LibreOffice

### Easy Effects

- Overview: Applies real-time audio effects to PipeWire application streams, such as equalization and compression.
- Website: https://wwmm.github.io/easyeffects
- Source Code: https://github.com/wwmm/easyeffects
- Download: https://flathub.org/en/apps/com.github.wwmm.easyeffects

### Ruffle

- Overview: A Flash Player emulator that runs Flash content on modern systems and browsers via Rust and WebAssembly.
- Website: https://ruffle.rs/
- Source Code: https://github.com/ruffle-rs/ruffle
- Download: https://flathub.org/en/apps/rs.ruffle.Ruffle

### Okular

- Overview: A universal document viewer for reading and annotating PDFs, eBooks, comics, and image files.
- Website: https://okular.kde.org/
- Source Code: https://invent.kde.org/graphics/okular
- Download: https://flathub.org/en/apps/org.kde.okular

### Kdenlive

- Overview: Provides non-linear video editing for assembling and refining video projects with multi-track timelines and effects.
- Website: https://kdenlive.org/
- Source Code: https://invent.kde.org/multimedia/kdenlive
- Download: https://flathub.org/en/apps/org.kde.kdenlive

### MKVToolNix

- Overview: Provides tools to create, alter, and inspect Matroska (MKV) container files.
- Website: https://mkvtoolnix.download/
- Source Code: https://codeberg.org/mbunkus/mkvtoolnix
- Download: https://flathub.org/en/apps/org.bunkus.mkvtoolnix-gui

### Inkscape

- Overview: A professional vector graphics editor for creating and editing scalable vector imagery, primarily using SVG.
- Website: https://inkscape.org/
- Source Code: https://gitlab.com/inkscape/inkscape
- Download: https://flathub.org/en/apps/org.inkscape.Inkscape

### Blockbench

- Overview: A low-poly 3D model editor for creating and animating textured models, especially for Minecraft and games.
- Website: https://www.blockbench.net/
- Source Code: https://github.com/JannisX11/blockbench
- Download: https://www.blockbench.net/downloads

### LMMS

- Overview: A digital audio workstation for composing, arranging, and mixing music using built-in instruments, effects, and sample support.
- Website: https://lmms.io/
- Source Code: https://github.com/LMMS/lmms
- Download: https://lmms.io/download#linux

### Tenacity

- Overview: A multi-track audio editor and recorder for editing and recording multi-track audio.
- Website: https://tenacityaudio.org/
- Source Code: https://codeberg.org/tenacityteam/tenacity/
- Download: https://flathub.org/en/apps/org.tenacityaudio.Tenacity

### Natron

- Overview: A node-based compositor for visual effects and digital post-production work.
- Website: https://natrongithub.github.io/
- Source Code: https://github.com/NatronGitHub/Natron
- Download: https://natrongithub.github.io/#download
